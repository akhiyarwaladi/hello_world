...
Malaria remains a critical global health challenge with over 200 million annual cases, yet traditional microscopic diagnosis is time-consuming and requires expert pathologists. This study proposes a multi-model hybrid framework combining YOLO (v10-v12) for detection and six CNN architectures (DenseNet121, EfficientNet-B0/B1/B2, ResNet50/101) for classification, validated on two public MP-IDB datasets comprising 418 images across 8 distinct classes (4 Plasmodium species: P. falciparum, P. vivax, P. malariae, P. ovale; and 4 lifecycle stages: ring, trophozoite, schizont, gametocyte). The proposed Option A architecture employs a shared classification approach where ground truth crops are generated once and reused across all detection methods, enabling efficient model training without redundant computation. YOLOv11 achieves competitive detection performance with 93.10% mAP@50 and 92.26% recall on species classification (MP-IDB Species dataset), and 92.90% mAP@50 with 90.37% recall on lifecycle stage detection (MP-IDB Stages dataset). For classification, EfficientNet-B1 achieves 98.80% accuracy with 93.18% balanced accuracy on species classification despite severe class imbalance (227 P. falciparum samples vs 5 P. ovale samples, 45:1 ratio), while EfficientNet-B0 achieves 94.31% accuracy with 69.21% balanced accuracy on lifecycle stages (272 ring vs 5 gametocyte samples, 54:1 ratio). A notable finding is that smaller EfficientNet models (5.3-7.8M parameters) consistently outperform larger ResNet variants (25.6-44.5M parameters) on these small medical imaging datasets, with EfficientNet-B1 achieving 10.5 percentage points higher balanced accuracy than ResNet101 (93.18% vs 82.73%) despite having 5.7× fewer parameters, challenging the conventional "deeper is better" paradigm. The system addresses extreme class imbalance through optimized Focal Loss (α=0.25, γ=2.0), achieving minority class F1-scores of 76.9% on P. ovale (5 samples) with perfect recall, 92.3% on schizont (7 samples), and 57.1% on gametocyte (5 samples), though performance on trophozoite minority class (15 samples) remains challenging at 51.6% F1-score. The proposed framework demonstrates practical feasibility for malaria diagnosis with consumer-grade GPU hardware (NVIDIA RTX 3060), though clinical deployment requires further validation on larger datasets and field-collected samples.
...
This study addresses these challenges through a novel multi-model hybrid framework with a shared classification architecture. Our approach trains classification models once on ground truth crops and reuses them across multiple YOLO detection methods, enabling computational efficiency through elimination of redundant training across detection backends. We validate our system on two public MP-IDB (Malaria Parasite Image Database) datasets covering both species classification (4 Plasmodium species) and lifecycle stage classification (4 stages: ring, trophozoite, schizont, gametocyte), totaling 418 images with severe class imbalance (ratios up to 54:1 for ring vs gametocyte stages).
...
### 3.1 Detection Performance and Task-Dependent Patterns

YOLO detection models demonstrated competitive performance across both MP-IDB datasets, with all three variants achieving mAP@50 exceeding 90%, as presented in Table 2. On the MP-IDB Species dataset, YOLOv12 achieved the highest mAP@50 at 93.12%, closely followed by YOLOv11 (93.10%) and YOLOv10 (92.53%), indicating marginal differences among model versions for this task. However, YOLOv11 demonstrated superior recall (92.26%) compared to YOLOv12 (91.18%) and YOLOv10 (89.57%), making it the preferred choice for clinical deployment where false negatives (missed parasites) are more critical than false positives. Training times ranged from 1.8 hours (YOLOv10) to 2.1 hours (YOLOv12), reflecting the increasing architectural complexity of newer YOLO versions.
...
**File Paths for Figure 9**:
- Figure 9(a) Stages Detection GT: `results/optA_20251007_134458/experiments/experiment_mp_idb_stages/detection_classification_figures/det_yolo11_cls_efficientnet_b1_focal/gt_detection/1704282807-0021-T_G_R.png`
- Figure 9(a) Stages Detection Pred: `pred_detection/1704282807-0021-T_G_R.png`
- Figure 9(b) Stages Classification GT: `gt_classification/1704282807-0021-T_G_R.png`
- Figure 9(b) Stages Classification Pred: `pred_classification/1704282807-0021-T_G_R.png`
- Figure 9(c) Species Detection GT: `results/optA_20251007_134458/experiments/experiment_mp_idb_species/detection_classification_figures/det_yolo11_cls_efficientnet_b1_focal/gt_detection/1704282807-0012-R_T.png`
- Figure 9(c) Species Detection Pred: `pred_detection/1704282807-0012-R_T.png`
- Figure 9(d) Species Classification GT: `gt_classification/1704282807-0012-R_T.png`
- Figure 9(d) Species Classification Pred: `pred_classification/1704282807-0012-R_T.png`

### 3.4 Computational Efficiency and Deployment Feasibility
...
The proposed framework is designed for deployment on consumer-grade GPU hardware (NVIDIA RTX 3060 with 12GB VRAM) or modern multi-core CPU systems with 32GB RAM, positioning it as accessible for resource-constrained healthcare settings common in malaria-endemic regions. Modern YOLO architectures typically achieve inference speeds of 10-20ms per image on similar GPU hardware based on published benchmarks [13,14], substantially faster than traditional microscopic examination which requires 20-30 minutes per slide [6]. The two-stage pipeline design (detection followed by classification) enables parallel processing of multiple parasites per image, though future single-stage multi-task learning approaches could further optimize end-to-end latency [39].
...
Third, both MP-IDB datasets originated from controlled laboratory settings with standardized protocols. External validation on field-collected samples with varying staining quality and diverse microscope types is essential to assess real-world generalization. Planned collaboration with hospitals in endemic regions will provide 500+ diverse clinical samples for Phase 2 validation, testing robustness to domain shift [38]. Fourth, while the current two-stage pipeline (detection followed by classification) enables modular training and optimization, single-stage multi-task learning approaches [39] could potentially improve accuracy through joint feature learning and simplify deployment. Fifth, while Grad-CAM visualizations [40] provide qualitative insights into model attention patterns, quantitative validation against expert annotations is needed to verify that models learn clinically relevant features rather than spurious correlations.

---

## 4. CONCLUSION

This study presents a multi-model hybrid framework for automated malaria detection and classification, validated on two public MP-IDB datasets (418 images, 8 classes across Plasmodium species and lifecycle stages). The proposed Option A architecture employs a shared classification approach that trains CNN models once on ground truth crops and reuses them across multiple YOLO detection methods, enabling computational efficiency without sacrificing accuracy. YOLOv11 detection achieves 93.10% mAP@50 with 92.26% recall on species classification and 92.90% mAP@50 with 90.37% recall on lifecycle stages, while EfficientNet-B1 classification reaches 98.80% accuracy (93.18% balanced accuracy) on species and EfficientNet-B0 achieves 94.31% accuracy (69.21% balanced accuracy) on lifecycle stages despite severe class imbalance.
...
Implemented on consumer-grade GPU hardware (NVIDIA RTX 3060), the system demonstrates practical feasibility for deployment in resource-constrained healthcare settings common in malaria-endemic regions. Future work will focus on dataset expansion to 1000+ images through synthetic data generation and clinical collaborations, inference speed optimization through model quantization, and external validation on field-collected samples to assess real-world generalization beyond controlled laboratory datasets. The combination of competitive accuracy, computational efficiency through shared classification, and modest hardware requirements positions this framework as a promising tool for democratizing AI-assisted malaria diagnosis in resource-limited settings.
...
- **Structure**: Results and Discussion INTEGRATED (Section 3)
  - Section 1: Introduction
  - Section 2: Materials and Methods
  - Section 3: Results and Discussion (integrated, 5 subsections)
...
- Tables: 4 (all with placeholders and file paths)
  - Table 1: Dataset statistics (`luaran/tables/Table3_Dataset_Statistics_MP-IDB.csv`)
  - Table 2: Detection performance (`luaran/tables/Table1_Detection_Performance_MP-IDB.csv`)
  - Table 3: Classification performance (`luaran/tables/Table2_Classification_Performance_MP-IDB.csv`)
  - Table 4: Per-class metrics (`luaran/tables/Table9_MP-IDB_Species_Focal_Loss.csv`, `Table9_MP-IDB_Stages_Focal_Loss.csv`)
- References: 40 citations [1]-[40] maintained throughout

**Formatting Notes:**
- All text in narrative paragraph format (no bullet points except contributions list)
- Figures/tables explicitly mentioned in text flow ("as shown in Table 3", "Figure 5 illustrates", etc.)
- All placeholders marked with [INSERT X HERE] include file paths
- File paths provided for all visual elements
- All reference citations [1]-[40] preserved during integration
