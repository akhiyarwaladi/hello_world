...
**Image Selection:** ✅ Based on IMAGE_SELECTION_REPORT.md with verified metrics (2025-01-27)
...
Systematic comparison of YOLO variants (v10/v11/v12 Medium, 20.1M parameters) reveals dataset-dependent performance patterns across all four malaria datasets (Table 2). YOLO11 achieves balanced best performance with 94.99% mAP@50 on IML Lifecycle and 72.91% on challenging MD_2019, while YOLO12 demonstrates superiority on severe imbalance scenarios reaching 96.27% mAP@50 on MP-IDB Stages. YOLO10 provides competitive baseline performance ranging 70.84-93.81% mAP@50, validating incremental improvements across YOLO generations for medical imaging tasks.
...
Table 6: Classification Performance on MD_2019 Stages Dataset (3 Lifecycle Stages, 883 Images from 16 Patients)
...
Transparent visualization of failure modes provides critical insights into system limitations and guides future improvements toward clinical deployment. We present color-coded detection errors (Figure 3) and classification confusion patterns (Figure 4) with balanced representation across all four datasets (2 images per dataset for both detection and classification) to honestly assess current capabilities while identifying systematic challenges requiring further research.

Detection visualizations employ color coding where green boxes indicate true positives (correct detections matching ground truth), red boxes mark false positives (incorrect predictions), and yellow boxes highlight false negatives (missed parasites), enabling immediate visual assessment of error types and severity across diverse microscopy conditions.

path: luaran/templates/figures/qualitative_detection/det1_iml_fp.png
Figure 3a: False Positive on IML Lifecycle - YOLOv11 showing 1 false positive (red) among 3 correct detections (green) demonstrating precision challenge

The IML Lifecycle false positive case reveals YOLOv11's occasional confusion between parasite-like artifacts and actual parasites, where background cellular structures such as platelet aggregates or staining artifacts morphologically resemble early-stage ring forms. This single false alarm among 3 correct detections (75% precision) represents typical performance on high-quality manually-annotated datasets where the detector achieves strong overall accuracy but occasionally hallucinates detections on ambiguous background regions.

The false positive likely stems from a cellular debris element exhibiting similar size, circular shape, and chromatin-like texture to genuine ring-stage parasites, demonstrating the fundamental challenge of distinguishing true parasites from morphologically similar blood components in complex microscopy fields. Such false alarms, while requiring manual verification in clinical workflows, represent an acceptable trade-off for maintaining high recall rates that minimize dangerous false negatives.

path: luaran/templates/figures/qualitative_detection/det2_iml_fn.png
Figure 3b: False Negative on IML Lifecycle - YOLOv11 missing single parasite (yellow) demonstrating recall challenge on subtle early-stage forms

The IML Lifecycle false negative case demonstrates the complementary challenge where YOLOv11 completely misses a single parasite despite correctly detecting others in the same field, revealing sensitivity limitations on subtle early-stage forms. The missed detection (yellow box) likely represents a faint ring-stage parasite with weak Giemsa staining intensity or small size occupying less than 5% of host cell volume, falling below YOLOv11's default confidence threshold of 0.25. This lifecycle-specific failure pattern emphasizes the critical importance of high recall in clinical deployment, as missed diagnoses directly translate to untreated patients and potential disease progression. The single miss among multiple correct detections suggests that stage-aware detection with adjusted confidence thresholds could improve early-stage sensitivity, trading slightly increased false positives for critical gains in catching subtle infections before progression to severe disease.

path: luaran/templates/figures/qualitative_detection/det3_stages_heavy_fp.png
Figure 3c: Heavy False Positive Scenario - YOLOv11 overdetection on MP-IDB Stages showing 8 false positive predictions (red) demonstrating extreme precision failure

The MP-IDB Stages overdetection case represents the most severe precision failure mode where 8 false positive predictions (red boxes) indicate systematic confusion between parasite-like artifacts and actual parasites in severely imbalanced data (54:1 ratio). This overdetection pattern, while maintaining high recall to minimize missed diagnoses, demonstrates the precision-recall trade-off inherent in medical detection systems where false alarms require secondary manual verification. The high false positive rate in this challenging sample reflects background clutter from cellular debris, platelet aggregates, and staining artifacts morphologically similar to ring-stage parasites that dominate the training data. Such extreme cases motivate future work on improved feature discrimination through attention mechanisms focusing on diagnostically relevant morphological characteristics and enhanced training data diversity to better distinguish true parasites from background noise [24].

path: luaran/templates/figures/qualitative_detection/det4_species_mixed.png
Figure 3d: Mixed Error Scenario - YOLOv11 on MP-IDB Species exhibiting both false positives (3 red) and false negatives (3 yellow) simultaneously

The MP-IDB Species mixed error case demonstrates the most challenging detection scenario where both overdetection (3 false positives) and underdetection (3 false negatives) occur simultaneously in a single image, resulting in 50% precision and 50% recall. This bidirectional failure mode typically emerges in crowded microscopy fields with overlapping cells where the detector struggles to correctly segment individual parasite boundaries, resulting in merged detections (causing false negatives for missed parasites) and split detections (causing false positives for background regions). The simultaneous occurrence of both error types highlights the fundamental difficulty in distinguishing tightly packed parasites from morphologically similar blood components, suggesting need for instance segmentation approaches that provide pixel-level parasite boundaries rather than bounding boxes.

path: luaran/templates/figures/qualitative_detection/det5_md2019_crowded_fp.png
Figure 3e: Crowded Field False Positives - YOLOv11 showing 2 false positives on MD_2019 multi-patient dataset demonstrating generalization challenge

The MD_2019 crowded field case with multiple detections including 2 false positives represents realistic clinical detection difficulty where natural inter-patient variation in parasite morphology, host cell characteristics, and sample preparation quality creates detection challenges absent from controlled single-source datasets. The false positives in this densely populated microscopy field with 10+ parasites demonstrate that YOLOv11's performance degrades in complex multi-parasite scenarios where overlapping cells and variable staining intensity confound the detector. This patient-specific failure aligns with the dataset's overall 72.91% mAP@50, honestly reflecting the performance degradation expected during real-world deployment across diverse patient populations with varying blood smear quality and motivating multi-center data collection to improve cross-site robustness [16].

path: luaran/templates/figures/qualitative_detection/det6_md2019_fn.png
Figure 3f: Multi-Patient False Negative - YOLOv11 missing single parasite on MD_2019 dataset reflecting inter-patient morphological variation

The MD_2019 false negative case from a different patient demonstrates consistent recall challenges across the multi-patient dataset, where the missed detection (yellow box) exhibits atypical morphology with irregular shape and diffuse chromatin distribution that diverges from training data appearance. This generalization gap between laboratory datasets (IML, MP-IDB) and field samples (MD_2019) with natural morphological diversity emphasizes the critical need for training data that captures full spectrum of parasite appearances across different patients, geographic regions, and microscopy protocols. The consistent pattern of single-parasite misses across MD_2019 samples suggests that ensemble approaches combining multiple detection models with voting schemes could reduce false negatives and improve overall recall on morphologically diverse field samples [5].

Classification error analysis on cropped parasites reveals systematic confusion patterns with balanced representation across all four datasets (Figure 4). To ensure fair comparison and focus on error pattern analysis rather than architecture comparison, all visualizations employ the best-performing model for each respective dataset: EfficientNet-B1 for IML Lifecycle and MP-IDB Species, EfficientNet-B0 for MD_2019 Stages, demonstrating error patterns representative of state-of-the-art performance.

path: luaran/templates/figures/qualitative_classification/cls1_iml_single.png
Figure 4a: Single Misclassification on IML Lifecycle - EfficientNet-B1 confusing trophozoite as ring showing subtle morphological ambiguity

The IML Lifecycle single error case represents the minimum error level achievable on moderately imbalanced data, where a single trophozoite exhibits atypically small cytoplasmic volume and compact morphology resembling ring-stage appearance. This isolated confusion with 66.7% accuracy on 3 parasites (1 error) demonstrates that even on high-quality manually-annotated datasets, borderline cases exist where parasites occupy morphological transition zones between discrete stage categories. The misclassified trophozoite likely represents an early-development specimen at 24-26 hours post-invasion where cytoplasmic maturation has not yet progressed sufficiently for confident trophozoite identification, highlighting the inherent subjectivity in lifecycle stage assignment that limits maximum achievable accuracy regardless of model architecture.

path: luaran/templates/figures/qualitative_classification/cls2_iml_moderate.png
Figure 4b: Moderate Error on IML Lifecycle - EfficientNet-B1 showing 1 error among 3 parasites demonstrating stage transition challenge

The IML Lifecycle moderate error case with 3 parasites and 1 misclassification (33.3% error rate) demonstrates typical performance on moderately imbalanced lifecycle data, where the model correctly identifies 2 parasites but confuses a single ring-stage parasite due to atypical morphology or staining characteristics. This moderate failure rate on a small cluster of parasites reflects the continuous nature of parasite development where early ring forms gradually transition into trophozoites through incremental cytoplasmic expansion without discrete morphological boundaries. The 66.7% accuracy on this challenging cluster suggests that while the framework achieves excellent overall performance (91.51% on IML), individual images with morphologically ambiguous specimens remain problematic and require human expert verification in clinical deployment.

path: luaran/templates/figures/qualitative_classification/cls3_stages_moderate.png
Figure 4c: Early-to-Mid Stage Confusion - EfficientNet-B1 misclassifying 4 trophozoites as rings on MP-IDB Stages highlighting lifecycle transition ambiguity

The MP-IDB Stages trophozoite-ring confusion with 4 errors demonstrates classification challenges on severely imbalanced data (54:1 ratio), where early ring forms (12-24 hours post-invasion) transition into mid-stage trophozoites (24-36 hours) through gradual cytoplasmic expansion without discrete morphological boundaries. The 4 misclassifications reflect the continuous nature of parasite development where classification into discrete stages represents an artificial discretization of a smooth morphological progression, making ground truth labels themselves somewhat arbitrary for transitional parasites. This pattern suggests that ordinal regression approaches treating lifecycle stages as ordered categories rather than independent classes might better capture the underlying biological progression and reduce confusion between adjacent developmental stages.

path: luaran/templates/figures/qualitative_classification/cls4_species_confusion.png
Figure 4d: Species Misidentification - EfficientNet-B1 confusing P. vivax with P. ovale on MP-IDB Species demonstrating inter-species morphological similarity

The MP-IDB Species P. vivax-to-P. ovale confusion represents the rarest but most clinically significant error pattern, where morphologically similar non-falciparum species require correct identification for appropriate treatment selection since P. vivax and P. ovale harbor dormant liver-stage hypnozoites requiring primaquine therapy not used for P. falciparum. The single species misclassification reflects genuine morphological overlap between P. vivax and P. ovale ring stages, which human microscopists also find challenging to distinguish without clinical context and sequential blood smear analysis. This error with only 7-9 test samples per minority species emphasizes the need for few-shot learning techniques such as prototypical networks and metric learning approaches that can learn robust species discriminators from limited examples [25].

path: luaran/templates/figures/qualitative_classification/cls5_md2019_heavy.png
Figure 4e: Heavy Classification Confusion - EfficientNet-B0 misclassifying 6 schizonts as trophozoites on MD_2019 Stages demonstrating mature-stage discrimination challenge

The MD_2019 heavy confusion case with 6 schizont-to-trophozoite misclassifications reveals the most common systematic error pattern across all lifecycle stage classification tasks, where morphologically transitional mature stages exhibit overlapping features that confound CNN discriminators. Schizonts represent the final pre-rupture stage containing 8-32 merozoites with visible segmentation patterns, while mature trophozoites display irregular amoeboid shapes with active cytoplasmic streaming, creating ambiguous intermediate morphologies during the trophozoite-to-schizont transition that challenge both human microscopists and deep learning models. The 6 errors in a single image (100% misclassification) demonstrate that this confusion pattern is not random noise but systematic bias where subtle morphological cues distinguishing late trophozoites from early schizonts remain unlearned despite 75 training epochs, motivating future work on attention mechanisms that explicitly focus on diagnostically relevant regions such as chromatin distribution patterns and cytoplasmic segmentation [24].

path: luaran/templates/figures/qualitative_classification/cls6_md2019_perfect.png
Figure 4f: Perfect Classification on Crowded Field - EfficientNet-B0 achieving 100% accuracy on 10 parasites from MD_2019 demonstrating system capability

The MD_2019 perfect classification case with 10 parasites (patient Trip 067) demonstrates that despite the systematic errors shown in Figure 4e, the framework is capable of achieving flawless performance on crowded microscopy fields containing diverse lifecycle stages when morphological features are sufficiently distinct. This 100% accuracy on a challenging multi-parasite image provides balanced assessment of system capabilities, showing that classification failures result from specific morphological ambiguities at stage boundaries rather than fundamental architectural inadequacy. The successful classification of ring, schizont, and trophozoite stages across varying sizes, staining intensities, and overlapping cell positions in a single dense field validates that EfficientNet-B0's compact 5.3M parameter architecture possesses sufficient discriminative capacity for clinical deployment when parasites exhibit clear stage-specific morphology. This honest presentation of both failures and successes emphasizes that the 86.45% overall accuracy on MD_2019 reflects genuine biological ambiguity in transitional morphologies rather than systematic model deficiency, guiding future research toward targeted improvements on morphologically ambiguous cases rather than wholesale architecture replacement.
