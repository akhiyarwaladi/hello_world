        ...
        try:
            ...
            if len(matches) > 0:
                ...
                for idx, row in matches.iterrows():
                    ...
                    print(f"     FP: {row['n_false_positives']}, FN: {row['n_false_negatives']}, TP: {row['n_true_positives']}")
                    if 'avg_confidence' in row:
                        print(f"     Avg Confidence: {row['avg_confidence']:.3f}")
                    if 'fp_per_image' in df.columns and 'fn_per_image' in df.columns:
                        print(f"     FP/img: {row.get('fp_per_image', 'N/A')}, FN/img: {row.get('fn_per_image', 'N/A')}")
    ...
    if len(all_matches) == 0:
        ...
        for model in DETECTION_MODELS:
            ...
            if csv_path.exists():
                try:
                    df = pd.read_csv(csv_path)
                    # Get top N by similarity (you can customize scoring)
        ...
        try:
            ...
            if len(matches) > 0:
                ...
                for idx, row in matches.iterrows():
                    ...
                    if 'avg_confidence_incorrect' in df.columns:
                        print(f"     Avg Conf (Incorrect): {row.get('avg_confidence_incorrect', 'N/A')}")
    ...
    if len(all_matches) == 0:
        ...
        for model in models_to_search:
            ...
            if csv_path.exists():
                try:
                    df = pd.read_csv(csv_path)
                    # Sort by closeness to criteria and get top N
                    alternatives = df.head(top_n)
...
# Figure 5d: MP-IDB Species - mixed error, 1.06 FP/img, 0.28 FN/img, avg conf 0.685
def filter_5d(df):
    # Calculate per-image ratios if columns exist
    if 'fp_per_image' in df.columns and 'fn_per_image' in df.columns:
        return df[
            (df['n_false_positives'] > 0) &
            (df['n_false_negatives'] > 0) &
            (df['fp_per_image'].between(1.0, 1.1)) &
            (df['fn_per_image'].between(0.25, 0.30)) &
            (df['avg_confidence'].between(0.68, 0.69))
        ]
    else:
        # Fallback: just mixed errors
        return df[(df['n_false_positives'] > 0) & (df['n_false_negatives'] > 0)]

search_detection_case(
    "mp_idb_species",
    "Figure 5d: Mixed error (FP+FN, 1.06 FP/img, 0.28 FN/img, conf 0.685)",
    filter_5d
)

# Figure 5e: MD-2019 - crowded case, 0.48 FN/img, conf 0.740
def filter_5e(df):
    if 'fn_per_image' in df.columns:
        return df[
            (df['n_false_negatives'] > 0) &
            (df['fn_per_image'].between(0.45, 0.50)) &
            (df['avg_confidence'].between(0.73, 0.75))
        ]
    else:
        return df[(df['n_false_negatives'] > 0) & (df['n_false_positives'] > 0)]

search_detection_case(
    "md_2019_stages",
    "Figure 5e: Crowded case (mixed FP+FN, 0.48 FN/img, conf 0.740)",
    filter_5e
)
